...
def calculate_features(file_path, progress_callback, run_analysis=False):
    ...
    script_dir = sys.path[0]
    ml_model_path = os.path.join(script_dir, "MLmodel.pkl")        
    ...
    results['Prediction'] = results['Prediction'].apply(lambda x: 'non_vaccine_target' if x == -1 else 'vaccine_candidate')
    predictions_path = os.path.join(script_dir, "vaccine_targets.csv")
